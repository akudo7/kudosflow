    ...
    this._panel.webview.onDidReceiveMessage(
      async (message) => {
        switch (message.command) {
          case "getIconPath":
            const iconPath = this._panel.webview.asWebviewUri(
              Uri.joinPath(
                extensionUri,
                "resources",
                "icons",
            ...
            break;
          case "error":
            window.showErrorMessage(`${message.target}: ${message.value}`);
            break;
        }
      },
      undefined,
    ...
    return /*html*/ `
      ...
      <html lang="en">
        <head>
          ...
          <meta content="default-src 'none'; style-src ${webview.cspSource} 'nonce-${nonce}'; font-src ${webview.cspSource}; script-src 'nonce-${nonce}';">
          <link rel="stylesheet" type="text/css" href="${stylesUri}">
          <title>kudos-gpt</title>
          <style nonce="${nonce}">
            @font-face {
              font-family: "codicon";
              font-display: block;
